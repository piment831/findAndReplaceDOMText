...
class Finder {
    ...
    search() {
        const matchAggregation = (textAggregation) => {
    ...
    processMatches() {
        ...
        let startPortion, endPortion, innerPortions = [], curNode = node, match = matches.shift(), atIndex = 0, // i.e. nodeAtIndex
        matchIndex = 0, portionIndex = 0, doAvoidNode, nodeStack = [node];
        out: while (true) {
    ...
    replaceMatch(match, startPortion, innerPortions, endPortion) {
        ...
        if (matchStartNode === matchEndNode) {
            ...
            precedingTextNode = this.doc.createTextNode(matchStartNode.data.substring(0, startPortion.indexInNode));
            followingTextNode = this.doc.createTextNode(matchEndNode.data.substring(endPortion.endIndexInNode));
    ...
    getPortionReplacementNode(portion, match) {
        ...
        replacement = this.doc.createTextNode(this.prepareReplacementString(replacement, portion, match));
        if (!replacement.data) {
            return replacement;
        }
        if (!el) {
    ...
    prepareReplacementString(string, portion, match) {
        ...
        string = string.replace(/\$(\d+|&|`|')/g, function (t) {
            let replacement;
            switch (t) {
                case '&':
                    replacement = match[0];
...
Finder.PORTION_MODE_RETAIN = 'retain';
Finder.PORTION_MODE_FIRST = 'first';
Finder.NON_PROSE_ELEMENTS = {
    br: 1, hr: 1,
...
Finder.NON_CONTIGUOUS_PROSE_ELEMENTS = {
...
Finder.NON_INLINE_PROSE = function (el) {
